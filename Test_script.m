...
% will have the following useful information from our participants:
...
[n,t,rawdata] = xlsread('Thesis_Data_0331.csv'); % This converts the CSV file into a matlab file. Data is kept in data file.
...
for ii = 1:(m-3); % Go down all the rows minus the ones that had text in rawdata.
    ...
    if test == 1;  
        save = rawdata(ii+3,:); % Save the row if the test is passed. (If 0, fail.)
        tempdata = [tempdata; save]; % Concatenate new columns.
    end
end
...
for ii = 1:length(headers); % Go through every column in headers.
    ...
    if TF == 1; % If true
        save = tempdata(:,ii); % Save the engagement column
       EngageTest = [save, EngageTest]; % And concatenate the engagement columns together.
    end    
end     
...
for ii = 1:m; % Go through all the rows in tempdata
    ...
    if test == 0; % Test to see if the row has any flags. If no flags, then good.
        save = tempdata(ii,:); % Save unflagged rows.
        progressdata = [progressdata; save];
    end    
end

data = [headers; progressdata]; % Add the headers back so we know what we are working with.
...
start = find(strcmp('Mach_questions_1',data(1,:)));
finish = find(strcmp('Mach_questions_20',data(1,:)));
N = 20; % Number of questions
IndexedColumns = round(linspace(start,finish, N));
Mach_data = data(:,IndexedColumns);
Mach_data = cell2mat(Mach_data(2:end,:)); % This is the subset of data we will use.
...
start = find(strcmp('PNR_Scale_1',data(1,:)));
...
start = find(strcmp('UG_A_1_1',data(1,:)));
finish = find(strcmp('UG_A_25_1',data(1,:)));
N = (finish-start); % Number of questions
IndexedColumns = round(linspace(start,finish, N));
UG_data_temp = data(:,IndexedColumns);
UG_data_temp = cell2mat(UG_data_temp(2:end,:)); % This is the subset of data we will use.



% At this point, all you need to do is average out all of the rows for the
% average offer per subject. You don't need to worry about the zeroes.
% Nevermind because it saved the timing questions... so this doesn't work.

%% Clean UG-A Data 

UG_A_temp = [];

for ii = 1:length(headers); % number columns
    pattern = "UG_A_";
    TF = contains(headers{ii},pattern);
    if TF == 1;
        headers{ii};
        save = data(:,ii);
        UG_A_temp = [save, UG_A_temp];
    end    
end     
    
UG_A = cell2mat(UG_A_temp(2:end,:));
    
UG_A_Mean_Offers = mean(UG_A');
...
for ii = 1:length(headers); % number columns
    ...
    if TF == 1;
        headers{ii};
        save = data(:,ii);
        UG_B_temp = [save, UG_B_temp];
    end    
end     
    
UG_B = cell2mat(UG_B_temp(2:end,:));
...
UG_Fair_Column = find(strcmp('UG_Fair_1',data(1,:)));
UG_Fair = data(:,UG_Fair_Column);

UG_Optimal_Column = find(strcmp('UG_Optimal_1',data(1,:)));
