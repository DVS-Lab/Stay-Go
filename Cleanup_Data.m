...
[n,t,rawdata] = xlsread('Thesis_Data_0403.csv'); % This converts the CSV file into a matlab file. 

load order.mat

rawdata = [rawdata,FL_163_DO];

% This throws in the order of the survey for analysis at the end.
...
use = rawdata(1,:); % Pulls out the headers 
...
for ii = 1:m
    row = FinalData3(ii,:); % Go through each row.
    pattern = "FL_155|FL_164"; 
    TF = contains(row{end},pattern);% Test if subject is 'FL_155|FL_164' 
     if TF == 1;
         saveme = [ii,0]; % If yes, save subject row, 0
         UG_First = [UG_First; saveme];
     end    
     
    pattern2 = "FL_164|FL_155";
    TF2 = contains(row{end},pattern2);% Test if subject is {'FL_164|FL_155'}
     if TF2 == 1;
        saveme2 = [ii,1]; % If yes, save subject row, 1
        DG_First = [DG_First; saveme2];
     end
end 
...
save('Behavioral_Survey_Data', 'DG_Optimal', 'DG_Fair', 'DG_Ask', 'UG_Optimal', 'UG_Fair', 'UG_Ask', 'UG_First', 'DG_First')
